...
If the browser provides a heading value (compass bearing in degrees from north), a separate heading indicator icon is rendered at the same position, rotated to match the bearing. If heading data is unavailable or `null`, only the position marker is shown.

> Both markers use icons from the SVG sprite. Placeholder icons are used until the sprite is implemented.
